...
void ByteCodeBlock::fillLocDataIfNeeded(Context* c)
{
    ...
    if (m_codeBlock->asInterpretedCodeBlock()->isGlobalScopeCodeBlock()) {
        RefPtr<ProgramNode> nd = esprima::parseProgram(c, m_codeBlock->asInterpretedCodeBlock()->src(), nullptr, m_codeBlock->asInterpretedCodeBlock()->isStrict(), m_codeBlock->asInterpretedCodeBlock()->inStatic(), false, SIZE_MAX);
